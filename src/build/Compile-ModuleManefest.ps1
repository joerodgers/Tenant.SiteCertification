...
$classes = Get-ChildItem -Path $PSScriptRoot\..\Tenant.SiteCertification\private\classes\*.ps1
# C:\_projects\Tenant.SiteCertification\src\Tenant.SiteCertification\private\classes\Notification.ps1

foreach( $class in $classes )
{
    $content = Get-Content -Path $class.FullName -Raw

    $null = $sb.AppendLine( "# class file - src\Tenant.SiteCertification\private\classes\$($class.Name)" )
...
$sb.ToString() | Set-Content -Path "$PSScriptRoot\..\Tenant.SiteCertification\Tenant.SiteCertification.psm1"
